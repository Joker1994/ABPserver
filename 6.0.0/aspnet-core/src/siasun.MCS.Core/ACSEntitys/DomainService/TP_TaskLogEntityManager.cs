...
namespace siasun.MCS.ACSEntitys.DomainService
{
    ...
    public class TP_TaskLogEntityManager :MCSDomainServiceBase, ITP_TaskLogEntityManager
    {
		...
		private readonly IRepository<TP_TaskLogEntity,long> _tP_TaskLogEntityRepository;

		/// <summary>
		/// TP_TaskLogEntity的构造方法
		/// 通过构造函数注册服务到依赖注入容器中
		///</summary>
	public TP_TaskLogEntityManager(IRepository<TP_TaskLogEntity, long> tP_TaskLogEntityRepository)	{
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<TP_TaskLogEntity> FindByIdAsync(long id)
        {
