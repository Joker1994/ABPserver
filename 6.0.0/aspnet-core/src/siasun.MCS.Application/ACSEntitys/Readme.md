...
- 前端代码生成的文件文件路径请在MCS.Application类库中生成的对应实体代码文件中的Client中。
...
如果你选择了**生成权限功能**，请打开MCS.Application类库中的MCSApplicationModule.cs类文件。
...
请打开MCS.Application类库中MCSApplicationModule.cs中的 PreInitialize 方法中:
...
打开EntityFrameworkCore类库在 **MCSDbContext**类文件中添加以下代码段：

```csharp
public DbSet<TB_AlarmLogEntity>  TB_AlarmLogEntitys { get; set; }
...
{
   modelBuilder.ApplyConfiguration(new TB_AlarmLogEntityCfg());
 }
...
```
Add-Migration AddNewTB_AlarmLogEntityEntity_Migration
```
...
生成的多语言内容
在MCS.Core类库中对应实体下的`douyuyan.md`文件中。
